...
class StackedTransformer(nn.Module):
    def __init__(self, response_size=47, treatment_size=2, d_model=512, d_ff=512, activation='gelu',
                 mix=True, d_layers=3, n_heads=8, e_layers=2, distil=True, repeat_t=0, hidden_r=512, hidden_t=512,
                 hidden_cnn=256, window_sizes=[3, 5, 7], num_classes=2, window=24, dropout=0):
    ...
    def forward(self, x_r, x_t, is_train=True, gen_length=None):
        ...
        r_dec_out = self.r_dec_embedding(x_r, None)
        r_dec_out = self.r_decoder(r_dec_out, enc_out, None, None)
        r_output = self.r_projection(r_dec_out)[:,:24,:]

        t_dec_out = self.t_dec_embedding(x_t, None)
        t_dec_out = self.t_decoder(t_dec_out, enc_out, None, None)
        t_output = self.t_projection(t_dec_out)[:,:24,:]
        ...
        # return x_r[:, -self.window:, :], x_t[:, -self.window:, :],cnn_result


class Transformer(nn.Module):
    def __init__(self, response_size=47, treatment_size=2, d_model=512, d_ff=512, activation='gelu',
                 mix=True, d_layers=2, n_heads=8, e_layers=3, distil=True, repeat_t=0, hidden_r=512, hidden_t=512,
                 hidden_cnn=256, window_sizes=[3, 5, 7], num_classes=2, window=12, dropout=0.3):
    ...
    def forward(self, enc_in, enc_mask, dec_in, dec_mask, out_len, is_train=True):
        if not is_train:
            dec_in = enc_in[:, :1, :].permute(1,0,2)
            enc_in = enc_in[:, :1, :].permute(1,0,2)
            out_list = []
