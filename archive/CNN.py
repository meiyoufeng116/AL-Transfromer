...
class ADRNN_new(nn.Module):
    def __init__(self, hidden_size=128, response_size=47, r_layers=2, r_dropout=0.5, treat_size=2, t_layers=2, t_dropout=0.5, dropout_layer=0):
...
class RNN(nn.Module):
    def __init__(self,hidden_size=64, num_layers=2, dropout=0.2):
        ...
        self.drop_layer = nn.Dropout(p=self.dropout)
        self.out = nn.Linear(self.hidden_size, 49)
...
def evaluate(model, data_x, data_y, mask, loss_fn):  # x (batch, time_step, input_size)
    ...
    Pre_y = torch.zeros(length, batch, data_x.shape[2]).cuda()
...
def accuracy(ground_truth, Pre_yc, mask):  # P: t  b  input(4)   mask:b t input
    ...
    return Accu
def recall(ground_truth, Pre_yc, mask):  # P: t  b  input(4)   mask:b t input
    correct = torch.mul(Pre_yc, ground_truth).sum()
    base = ground_truth.sum()
    Accu = correct.item() / base.item()
    return Accu
...
def evaluate_CNN_MultiLabel(C1, data_x_r, data_x_t, data_y, mask, loss_fn, windows=12):  # x (batch, time_step, input_size)
    batch = data_x_r.shape[0]
    length = data_y.shape[1]
    ground_truth = data_y.permute(2,1,0)[47:] # input(2) t b
    ground_truth = ground_truth.permute(1,2,0)  # t b input(2)
    ...
    C_input = torch.cat((data_x_r.permute(1,0,2),data_x_t.permute(1,0,2)),2) # t b input
    C_input = torch.cat((C_input,Pre_y.permute(1,0,2)), 0)  # t b input
    C_input = C_input[-(length+windows):-1] # t[-24:] b input  
    C_input = C_input.permute(1,0,2) # t[-24:] b input
    Yc1_pred = C1(C_input)  # -> b t c
    Yc1_pred = F.sigmoid(Yc1_pred)
    Yc_pred = (Yc1_pred>0.2).mul_(1) 
    ...
    Accu_r = accuracy(ground_truth.permute(2,1,0)[0], Yc_pred.permute(2,0,1)[0], ground_mask.permute(2,1,0)[0])
    Accu_t = accuracy(ground_truth.permute(2,1,0)[1], Yc_pred.permute(2,0,1)[1], ground_mask.permute(2,1,0)[1])
    Recall_r = recall(ground_truth.permute(2,1,0)[0], Yc_pred.permute(2,0,1)[0], ground_mask.permute(2,1,0)[0])
    Recall_t = recall(ground_truth.permute(2,1,0)[1], Yc_pred.permute(2,0,1)[1], ground_mask.permute(2,1,0)[1])

    
    #print()
    return Accu_r, Accu_t, Recall_r, Recall_t
...
def run_RNN_rt(iterator, model_r, model_t,path):
    if torch.cuda.is_available():
    ...
    for t in range(num_epochs):
        ...
        with torch.set_grad_enabled(False):
            ...
            for b, batch in enumerate(iterator):
                ...
                mask_t = mask_t.permute(2,1,0)
                eval_loss_r, yr, yr_pred = evaluate_rt(model_r, data_x_r, data_y_r, mask_r, loss_fn)
                eval_loss_t, yt, yt_pred = evaluate_rt(model_t, data_x_t, data_y_t, mask_t, loss_fn)
            ...
            for b, batch in enumerate(iterator):
                ...
                mask_t = mask.permute(2,1,0)[47:]
                mask_t = mask_t.permute(2,1,0)
                eval_loss_r, yr, yr_pred = evaluate_rt(model_r, data_xy_r, data_z_r, mask_r, loss_fn)
                eval_loss_t, yt, yt_pred = evaluate_rt(model_t, data_xy_t, data_z_t, mask_t, loss_fn)
                Y.append(torch.cat((yr,yt),2))
                Y_pred.append(torch.cat((yr_pred, yt_pred),2))
            l1_r,l1_t,l2_r,l2_t = RMSE(Y, Y_pred)
...
def run_ADRNN(iterator, model, path):
    if torch.cuda.is_available():
        model=model
    print(model)
        ...
        with torch.set_grad_enabled(False):
            ...
            for b, batch in enumerate(iterator):
                ...
                if torch.cuda.is_available():
                    ...
                    data_y = data_y.cuda()
                    mask = mask.cuda()
                eval_loss_r, eval_loss_t, y, y_pred = evaluate_ADRNN(model, data_x_r, data_x_t, data_y, mask, loss_fn)
                Y.append(y)
                Y_pred.append(y_pred)
            l1_r,l1_t,l2_r,l2_t = RMSE(Y, Y_pred)
            print("Epoch ", t,"  dev_loss_all: ", str(l1_r+l1_t+l2_r+l2_t),  "  valid_l1_r: ", l1_r, "  valid_l1_t: ", l1_t,  "  valid_l2_r: ", l2_r, "  valid_l2_t: ", l2_t)
            ...
            for b, batch in enumerate(iterator):
                ...
                if torch.cuda.is_available():
                    ...
                    data_z = data_z.cuda()
                    mask = mask.cuda()
                eval_loss_r, eval_loss_t, y, y_pred = evaluate_ADRNN(model, data_xy_r, data_xy_t, data_z, mask, loss_fn)
                Y.append(y)
                Y_pred.append(y_pred)
            l1_r,l1_t,l2_r,l2_t = RMSE(Y, Y_pred)
...
def run_CNN_MultiLabel(iterator, C1, window=12, path=None):
    print(C1)
    ...
    for t in range(num_epochs):
        ...
        for b, batch in enumerate(iterator):
            ...
            mask_c = mask[window:]  # b t input -> t b input
            mask_c = mask_c.permute(2,1,0)[-2:]  #   input b t 
            mask_c = mask_c.permute(1,2,0) # b t c
            Yc1_pred = torch.mul(Yc1_pred, mask_c.contiguous().view(-1,2)) # -1 c

            Target = batch[window:]  #-> t[24:]  b input
            Target = Target.permute(2,1,0)[47:] #-> input(2) b t[24:]
            Target = Target.permute(1,2,0) #-> b t[24:] c
            Target = score2Mlabel(Target.cuda()) # b t[24:] c
            Target = Target.view(-1,2) # -1 c
        ...
        print("Epoch ", t, "  train_loss_t1: ", train_loss_t1)
        file_w.write("Epoch "+str(t)+ "  train_loss_t1: "+str(train_loss_t1)+'\n')
        ...
        with torch.set_grad_enabled(False):
            for b, batch in enumerate(iterator):
                ...
                if torch.cuda.is_available():
                    ...
                    data_y = data_y.cuda()
                    mask = mask.cuda()
                accu_0, accu_1, recall_0, recall_1 = evaluate_CNN_MultiLabel(C1, data_x_r, data_x_t, data_y, mask, loss_fn, windows= window)
                valid_accu_0 += accu_0
                valid_accu_1 += accu_1
                valid_recall_0 += recall_0
                valid_recall_1 += recall_1
            print("Epoch ", t, " valid_accu_0: ", valid_accu_0, " valid_accu_1: ", valid_accu_1, " valid_recall_0: ", valid_recall_0, " valid_recall_1: ", valid_recall_1)
            file_w.write("Epoch "+str(t)+" valid_accu_0: "+str(valid_accu_0)+ " valid_accu_1: "+str(valid_accu_1)+ " valid_recall_0: "+str(valid_recall_0)+" valid_recall_1: "+str(valid_recall_1)+"\n")
            ...
            for b, batch in enumerate(iterator):
                ...
                if torch.cuda.is_available():
                    ...
                    data_z = data_z.cuda()
                    mask = mask.cuda()
                accu_0, accu_1, recall_0, recall_1  = evaluate_ADRNN_to_CNN_MultiLabel(model, C1, data_xy_r, data_xy_t, data_z, mask, loss_fn, windows= window)
                test_accu_0 += accu_0
                test_accu_1 += accu_1
                test_recall_0 += recall_0
                test_recall_1 += recall_1
            print("Epoch ", t, "  test_accu_0: ", test_accu_0, "  test_accu_1: ", test_accu_1,  "  test_recall_0: ", test_recall_0, "  test_recall_1: ", test_recall_1)
            file_w.write("Epoch "+str(t)+ "  test_accu_0: "+str(test_accu_0)+ "  test_accu_1: "+str(test_accu_1)+  "  test_recall_0: "+ str(test_recall_0)+ "  test_recall_1: "+str(test_recall_1))
    ...
    for b in batch_size:
        for feature_size in feature_sizes:
            ...
            params = {'batch_size': b,
                      'shuffle': True,
                      'num_workers': 6}
            dataset, mask = read_dataset('../code/dataset/normalized_dataset_new.pkl')
            ...
            if module == "LSTM_r_t":
                ...
                #model = ADRNN().cuda()
                model = ADRNN_new(h, 47, n, d, 2, tl, d, dropout_layer=0).cuda()
                ...
                classifier1 = Classifier(window_sizes = Kernel_size, feature_size=64, num_classes=2, window=w).cuda()##


            ######### Run ################
            path = 'model_cnn/'+str(module)+'_windows'+str(w)+'_batch'+str(b)+'_feature_size'+str(feature_size)+'_ Kernel_size'+str(Kernel_size)
            if module == "ADRNN_Linear" or module == "ADRNN":
                model = run_ADRNN(iterator, model,path)
            elif module == "LSTM_rt" or module == "LSTM_r_t":
                model = run_RNN_rt(iterator, model_r, model_t,path)
                ...
                model, c1 = run_ADRNN_to_CNN_MultiLabel(iterator, model, w, classifier1, path)
            elif module == "CNN":
                c1 = run_CNN_MultiLabel(iterator, classifier1, path)
            else:
