...
def run_RNN(train_iterator,vaild_iterator,test_iterator, model, path):
    if torch.cuda.is_available():
        model=model.cuda()
    print(str(model))
    ...
    for t in range(num_epochs):
        ...
        for b, batch in enumerate( train_iterator):
            batch, mask = batch  # batch = (batch, time_step, input_size)
        ...
        with torch.set_grad_enabled(False):
            ...
            for b, batch in enumerate(vaild_iterator):
                ...
                if torch.cuda.is_available():
                    ...
                    data_y = data_y.cuda()
                    mask = mask.cuda()
                eval_loss_r, eval_loss_t, y, y_pred = evaluate(model, data_y, data_y, mask, loss_fn)
                Y.append(y)
...
def run_RNN_rt(iterator, model_r, model_t):
    if torch.cuda.is_available():
    ...
    for t in range(num_epochs):
        ...
        with torch.set_grad_enabled(False):
            for b, batch in enumerate(iterator):
                ...
                mask_t = mask_t.permute(2,1,0)
                eval_loss_r = evaluate_rt(model_r, data_x_r, data_y_r, mask_r, loss_fn)
                eval_loss_t = evaluate_rt(model_t, data_x_t, data_y_t, mask_t, loss_fn)
            ...
            for b, batch in enumerate(iterator):
                ...
                mask_t = mask.permute(2,1,0)[47:]
                mask_t = mask_t.permute(2,1,0)
                eval_loss_r = evaluate_rt(model_r, data_xy_r, data_z_r, mask_r, loss_fn)
                eval_loss_t = evaluate_rt(model_t, data_xy_t, data_z_t, mask_t, loss_fn)
    ...
    for n in num_layers:
        for d in dropout:
            for b in batch_size:
                ...
                ######### Model ################
                if module == "RNN":
                    model = RNN(h, n, d).cuda()
                elif module == "LSTM":
                    model = LSTM(h, n, d).cuda()
                elif module == "GRU":
                    model = GRU(h, n, d).cuda()
                elif module == "LSTM_rt":
                    model_r = LSTM(h, n, d, 47).cuda()
                    model_t = LSTM(h, n, d, 2).cuda()
                elif module == "LSTM_r_t":
                    model_r = LSTM(h, n, d, 47).cuda()
                ...
                ######### Run ################
                path = '../trained_models/'+str(module)+'_'+str(h)+'_'+str(n)+'_'+str(d)+'_'+str(b)
                ...
                if module == "LSTM_rt" or module == "LSTM_r_t":
                    ...
                    model = run_RNN(train_iterator,vaild_iterator,test_iterator, model,path)
                torch.save(model, 'model_test/'+'LSTM12'+'.model')

######### Save ################
torch.save(model, '../trained_models/'+str(module)+'_'+str(hidden_size)+'_'+str(num_layers)+'_'+str(dropout)+'_'+str(batch_size)+'.model')
torch.save(c1, '../trained_models/'+str(module)+'_'+str(hidden_size)+'_'+str(num_layers)+'_'+str(dropout)+'_'+str(batch_size)+'.c')
